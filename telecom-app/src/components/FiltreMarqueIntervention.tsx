...
function FiltreMarqueIntervention() {
 const { marque } = useParams<{ marque: string }>();
  ...
  return (
    <div>
      <h2>Liste des interventions pour la marque "{marque}"</h2>
      <table className="table table-striped table-bordered">
        ...
        <tbody>
          {interventions.map((intervention: Intervention) => {
            if (intervention.marqueE === marque) {
              return (
...
export default FiltreMarqueIntervention;
